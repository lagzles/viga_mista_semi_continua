...
def inercia_equivalente_curta(tramo, le):
    ...
    relatorio = ""
    hdeck = tramo.hdeck
    largura_influencia = tramo.bInf # [m]
...
##    relatorio += "y0 = d/2 - yg + ycg_laje = {}\n".format(round(y0,2))
    ...
    relatorio += "itneracao da ligação = {}\n".format(interacao)
...
##    print(lista_Momentos_Zero)
    ...
    for tramo in lista_tramos:
        relatorio = ""
        n = tramo.n
        data_return = datab.get_data(dtb,n)
...
####        print('tds', tds)
##        print('mrdNegi', mrdNegi)
##        print('mrdNegf', mrdNegf)
##        print('d', d)
...
######        n = round(tds / Qrd,0) #  numero de conectores)
        ...
        if v7 > 1.03:
            relaorio += "Como 1 conector por onda não é suficiente, utilizaremos 2."
            num = 2
            n_conect *= num
            e_qrd_neg = (n_conect * 0.85 * Qrd)            
            v7 = tds / e_qrd_neg
            relaorio += "e_qrd = (n_conect * 0.85 * Qrd) = {}\n".format(round(e_qrd_neg,2))
            relaorio += "v7 = tds / e_qrd = {}\n".format(round(e_qrd_neg,2))

        if v7 > 1.03:
            relaorio += "Como 2 conectores por onda não é suficiente, utilizaremos 3."
            num = 3
            n_conect *= num
            e_qrd_neg = (n_conect * 0.70 * Qrd)
            v7 = tds / e_qrd_neg
            relaorio += "e_qrd = (n_conect * 0.70 * Qrd) = {}\n".format(round(e_qrd_neg,2))
            relaorio += "v7 = tds / e_qrd = {}\n".format(round(e_qrd_neg,2))
        ...
        vsd_pos = (e_qrd_pos - 100 * befPos*tc/2*fck/1.4)/ (vao_pos / 2)  # - Asl*fys - 100*befPos*tc/2*fck/1.4) / vao_pos
        ...
        sd_text = datab_g.get_data(dbg, 'SD')
        ...
        relatorio += "\n10 verificação - Limitação das tensões de serviço\n"
        relatorio += "(Combinação rara de serviços)\n"
        ...
        ftc = round(abs(mlNeg*100) / (d + ycg_laje),2)
        t2 = round(ftc / Asl,2)
        ...
        ##########################################################
        # 11 Verificação ##########################
        relatorio += "\n11ª Verificação - Flecha/Deformação\n"
        relatorio += "Não verificado atualmente. Esta em desenvolvimento.\n"
        # flecha antes da cura
        dac = (5 * (q_glinha) * vao**4) / (384 * e * (ia / 100**4)) * 100  #cm
        ...
        #flecha limite
        dadm = vao / 350 * 100
        dtotal = dac + dcpdc + dscdc
        
        cf_85_ac = 0.85 * dac * 10  # mm
        cf_70_tot_sc = 0.7 * (dtotal - dscdc) * 10 # mm
        cf_adotada = dac

        dfinal = dtotal - cf_adotada
        ...
        relatorio += "Acr = {} \n".format(acr)
        relatorio += "pri = ((3.14 * (bitola ** 2)) / 4) / acr = {} \n".format(pri)
        relatorio += "nn1 = 2.25 \n"
        relatorio += "fctm = 0.3 * (fck ** 0.66) = {} \n".format(fctm)
        relatorio += "tsi = (mlCurta * 0.4 + mlLonga)*100 / (d + (tc+hf-3.5))/ Asl = {} \n".format(tsi)
        relatorio += "w1 = {} \n".format(w1)
        relatorio += "w2 = {} \n".format(w2)
        ...
        classe_agressividade = datab_g.get_data(dbg, 'classe_agressividade')
        ...
        relatorio += "Valor limite de fissuração = {}mm \n".format(valor_limite)
...
##        print('Espaçamento i  %.2f' % (befNegi/nb))
##        print('Espaçamento f  %.2f' % (befNegf/nb))
