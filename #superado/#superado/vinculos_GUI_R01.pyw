from tkinter import *  # Label, Button, Checkbutton, IntVar, Tk, Entry
import tkinter as tk
from tkinter import messagebox
import dimensionamento_R02 as dim

# TODO criar entradas de vaos
# TODO vincular com rotina de analise
# TODO vincular com rotina de plotagem
# TODO Criar entry's de vaos responsivo com o 'optionmenu'
...
class Interface:
    ...
    def __init__(self, master):
        ...
        self.w = OptionMenu(master, variable, 1, 2, 3)
        ...
        # self.text_vaoad.insert(0, "1250.0")
...
##        self.label12 = Label(master, text='\t', height=1).grid(row=3, column=2)
        ##
        ## Dados Steel Deck
        self.label_hdeck = Label(master, text='Altura Laje [mm]=', height=1, justify=tk.LEFT).grid(row=1, column=1)
        self.text_hdeck = Entry(master, width=10)
        self.text_hdeck.insert(0, "150.0")
        self.text_hdeck.grid(row=1, column=2)

        self.label_ha = Label(master, text='Tamanho do apoio [cm]=', height=1, justify=tk.LEFT).grid(row=2, column=1)
        self.text_ha = Entry(master, width=10)
        self.text_ha.insert(0, "25.0")
        self.text_ha.grid(row=2, column=2)
        ##
        self.label5 = Label(master, text='\t', height=1).grid(row=3, column=1)
        self.label6 = Label(master, text='\t', height=1).grid(row=3, column=2)
        ## Dados da Armadura Negativa
        self.label_nas = Label(master, text='Numero de Barras [un]=', height=1, justify=tk.LEFT).grid(row=4, column=1)
        self.text_nas = Entry(master, width=5)
        self.text_nas.insert(0, "5")
        self.text_nas.grid(row=4, column=2)

        self.label_bitola = Label(master, text='Bitola Barras [mm]=', height=1, justify=tk.LEFT).grid(row=5, column=1)
        self.text_bitola = Entry(master, width=5)
        self.text_bitola.insert(0, "16.0")
        self.text_bitola.grid(row=5, column=2)

        #Dados perfil
        self.label_d = Label(master, text='Altura Perfil [cm]=', height=1, justify=tk.LEFT).grid(row=1, column=4)
        self.text_d = Entry(master,  width=10)
        self.text_d.insert(0, "57.0")
        self.text_d.grid(row=1, column=5)

        self.label_bf = Label(master, text='Mesa Perfil [cm]=', height=1, justify=tk.LEFT).grid(row=2, column=4)
        self.text_bf = Entry(master,  width=10)
        self.text_bf.insert(0, "17.5")
        self.text_bf.grid(row=2, column=5)

        self.label_tf = Label(master, text='tf [cm]=', height=1, justify=tk.LEFT).grid(row=3, column=4)
        self.text_tf = Entry(master,  width=10)
        self.text_tf.insert(0, "0.8")
        self.text_tf.grid(row=3, column=5)

        self.label_tw = Label(master, text='tw [cm]=', height=1, justify=tk.LEFT).grid(row=4, column=4)
        self.text_tw = Entry(master,  width=10)
        self.text_tw.insert(0, "0.635")
        self.text_tw.grid(row=4, column=5)
        ##
