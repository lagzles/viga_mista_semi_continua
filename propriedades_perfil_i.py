...
def i_y(d, tfs, bfs, bfi, tfi, tw):
    i_y = (tfs * bfs ** 3.0 / 12.0 + tfi * bfi ** 3 / 12.0 + (d - tfs - tfi) * tw ** 3 / 12.0)
    return i_y
...
def ycg(d, tw, bfs, tfs, bfi, tfi):
    a1 = bfi * tfi
    a2 = bfs * tfs
    hw = d - tfs - tfi
    a3 = hw * tw
    y = (a1* tfi*.5 + a2 * (bfs*.5+ hw + tfi) + a3*(d*.5)) / (a1+a2+a3)
    return y

def xcg(d, tw, bfs, tfs, bfi, tfi):
    x = cgy(bfs)
    return x
